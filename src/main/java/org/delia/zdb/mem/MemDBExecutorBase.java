...
public abstract class MemDBExecutorBase extends ServiceBase implements ZDBInternal {
	...
	protected QueryResponse doExecuteQuery(QuerySpec spec, QueryContext qtx) {
		...
		for(int i = 0; i < maxPreQueries; i++) {
			preQuerySpec = getPreQuery(spec, preQuerySpec, qresp0);
	...
	private QuerySpec getPreQuery(QuerySpec spec, QuerySpec preQuerySpec, QueryResponse qresp0) {
		if (preQuerySpec == null) {
			...
			//hack. we assume was the followers thing
			if (spec.queryExp.filter != null && spec.queryExp.filter.cond instanceof FilterOpFullExp) {
				FilterOpFullExp fofe = (FilterOpFullExp) spec.queryExp.filter.cond;
				if (fofe.isSingleQueryInExp()) {
					QueryInExp inExp = (QueryInExp) fofe.opexp1;
					...
					for(DValue inner: qresp0.dvalList) {
						...
						//TODO fix. create correct Exp type. for now we assume int
						IntegerExp exp = new IntegerExp(pk.asInt());
