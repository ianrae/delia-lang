...
public class IsDateOnlyRule extends ShapeSpecificRuleBase {
    private RuleOperand oper1;

    public IsDateOnlyRule(RuleGuard guard, RuleOperand oper1) {
        super("isDateOnly", guard);
        this.oper1 = oper1;
    }
    @Override
    public String renderAsDelia(RuleGeneratorContext ctx) {
        return String.format("%sisDateOnly()", ctx.getOperAsDelia(oper1));
    }
