...
public class RelNNTests extends NewHLSTestBase {
	...
	public void testFetch11Parent() {
		...
		chkFullSql(hld, "SELECT t0.cid,t0.x,t1.rightv,t2.id,t2.y,t1.leftv FROM Customer as t0 LEFT JOIN CustomerAddressDat1 as t1 ON t0.cid=t1.leftv JOIN Address as t2 ON t1.rightv=t2.id WHERE t0.cid=?", "55");
	}	
	@Test
	public void testFetch11Child() {
		...
		HLDQueryStatement hld = buildFromSrc(src, 1); 
		chkFullSql(hld, "SELECT t0.id,t0.y,t1.leftv,t2.cid,t2.x,t1.rightv FROM Address as t0 LEFT JOIN CustomerAddressDat1 as t1 ON t0.id=t1.rightv JOIN Customer as t2 ON t1.leftv=t2.cid WHERE t0.id=?", "100");
	}	
	...
	public void testImplicitFilter() {
		...
		HLDQueryStatement hld = buildFromSrc(src, 1); 
		chkFullSql(hld, "SELECT t0.cid,t0.x FROM Customer as t0 LEFT JOIN CustomerAddressDat1 as t1 ON t0.cid=t1.leftv JOIN Address as t2 ON t1.rightv=t2.id WHERE t2.y = ?", "55");
	}
	@Test
	public void testImplicitFilter2() {
		...
		HLDQueryStatement hld = buildFromSrc(src, 1); 
		chkFullSql(hld, "SELECT t0.cid,t0.x FROM Customer as t0 LEFT JOIN CustomerAddressDat1 as t1 ON t0.cid=t1.leftv JOIN Address as t2 ON t1.rightv=t2.id WHERE t2.y = ? ORDER BY t1.rightv", "55");
	}
