...
public class HSQL11Tests extends HLSTestBase {
	...
	public void testOneSpanSubSQL() {
		useCustomerSrc = true;
		sqlchk("let x = Customer[55].fks()", 					"SELECT a.id,a.x,b.id FROM Customer as a JOIN Address as b ON a.id=b.cust WHERE a.ID=55");
		sqlchk("let x = Customer[true].fetch('addr')", 			"SELECT a.id,a.x,b.id,b.y FROM Customer as a JOIN Address as b ON a.id=b.cust");
		sqlchk("let x = Customer[true].fetch('addr').first()", 	"SELECT TOP 1 a.id,a.x,b.id,b.y FROM Customer as a JOIN Address as b ON a.id=b.cust");
		sqlchk("let x = Customer[true].fetch('addr').orderBy('id')", "SELECT a.id,a.x,b.id,b.y FROM Customer as a JOIN Address as b ON a.id=b.cust ORDER BY a.id");
